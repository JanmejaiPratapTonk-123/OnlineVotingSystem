...
const Login = () => {
  ...
  const handleVerifyOTP = async (e) => {
    ...
    try {
      const response = await verifyOTP({ email: formData.email, otp });  // API call: POST /api/auth/verify-otp
      if (response.token) {
        login(response.user, response.token);  // Set auth context
      ...
      <div className="max-w-md w-full space-y-8">
        <div>
          ...
          <p className="mt-2 text-center text-sm text-gray-600 dark:text-gray-400">
            Securely log in to cast your vote
          </p>
        </div>
        <form className="mt-8 space-y-6" onSubmit={step === 1 ? handleLogin : handleVerifyOTP}>
          {step === 1 ? (
            <>
              ...
              <div>
                ...
                >
                  {loading ? 'Logging in...' : 'Login & Send OTP'}
                </button>
              </div>
            </>
          ) : (
